...
export class BackendResolver extends Context.Tag("BackendResolver")<
    BackendResolver,
    {
        ...
        ) => Effect.Effect<A, E | Error, Exclude<R, Backend> | AppConfig | S3Client>
...
export const BackendResolverLive = Layer.effect(
    ...
    Effect.gen(function* () {
        ...
        return {
            provideForBucket: <A, E, R>(bucketName: string, effect: Effect.Effect<A, E, R>) =>
                Effect.gen(function* () {
                    if (bucketCache.has(bucketName)) {
                        return yield* Effect.provideService(effect, Backend, bucketCache.get(bucketName)!)
                    }

                    const matched = config.lookupBucket(bucketName)
                    if (Option.isNone(matched)) {
                        return yield* Effect.fail(new Error(`No configuration found for bucket: ${bucketName}`))
                    }
                    ...
                    if (bucketConfig.protocol === "s3") {
                        ...
                        return yield* Effect.fail(new Error(`Unsupported protocol: ${bucketConfig.protocol}`))
                    }

                    bucketCache.set(bucketName, backendImpl)
                    return yield* Effect.provideService(effect, Backend, backendImpl)
                }) as Effect.Effect<A, E | Error, Exclude<R, Backend> | AppConfig | S3Client>,

            provideForBackendId: <A, E, R>(backendId: string, effect: Effect.Effect<A, E, R>) =>
                Effect.gen(function* () {
                    if (backendCache.has(backendId)) {
                        return yield* Effect.provideService(effect, Backend, backendCache.get(backendId)!)
                    }

                    const backendConfig = config.raw.backends[backendId]
                    if (!backendConfig) {
                        return yield* Effect.fail(new Error(`No configuration found for backend: ${backendId}`))
                    }
                    ...
                    if (backendConfig.protocol === "s3") {
                        ...
                        return yield* Effect.fail(new Error(`Unsupported protocol: ${backendConfig.protocol}`))
                    }
