  ...
  return {
    listObjects: (args: {
      ...
      Effect.gen(function* () {
        if (args.listType === 2) {
            ...
            try: () =>
              client.send(
                new ListObjectsV2Command({
                  ...
                  // An empty ContinuationToken is a no-op per S3; MinIO rejects
          ...
          const contents = (result.Contents ?? []).map(
            (c): ObjectInfo => ({
          ...
          const commonPrefixes = (result.CommonPrefixes ?? []).map(
            (cp): CommonPrefix => ({
          ...
          // S3's NextMarker is the last key or common prefix returned. MinIO
          // appends internal cache metadata (e.g. "[minio_cache:v2,return:]") to
          // its NextMarker, which must not leak into the S3 response, so derive
          // the marker from the last returned item instead.
          const lastItem = (() => {
            const lastContent = contents[contents.length - 1]?.key;
            const lastPrefix = commonPrefixes[commonPrefixes.length - 1]
              ?.prefix;
            if (lastContent === undefined) return lastPrefix;
            if (lastPrefix === undefined) return lastContent;
            return lastContent > lastPrefix ? lastContent : lastPrefix;
          })();
          const nextMarker =
            (result.IsTruncated ?? false) && lastItem !== undefined
              ? lastItem
              : undefined;
        ...
        const getWebStream = (): ReadableStream<Uint8Array> => {
          if (
            ...
            if (typeof b.transformToWebStream === "function") {
              return b.transformToWebStream() as ReadableStream<Uint8Array>;
            }
          }
          ...
          ? yield* Stream.runCollect(validatedStream).pipe(
            ...
            Effect.mapError((e) => {
              ...
              return new InternalError({
                message: `error collecting body stream into memory: ${
                  String(e)
                }`,
              });
            }),
    ...
    deleteObject: (key: string) =>
      Effect.gen(function* () {
        ...
        // Remove the persisted ACL for the object so it does not orphan
        // internal state (and block bucket deletion).
        yield* Effect.tryPromise({
          try: () =>
            client.send(
              new DeleteObjectCommand({
                Bucket: bucketName,
                Key: aclObjectKey(key),
              }),
            ),
          catch: (e) => mapS3Error(e, bucketName),
        }).pipe(Effect.ignore);
    ...
    copyObject: (
      ...
      Effect.gen(function* () {
        ...
        const { s3Params, metadata } = headerService.fromRequestHeaders(
          headers,
        );
    ...
    getObjectAcl: (key: string) =>
      Effect.gen(function* () {
        const owner = yield* getOwner();
        const stored = yield* readStoredPolicy(
          client,
          bucketName,
          aclObjectKey(key),
        );
