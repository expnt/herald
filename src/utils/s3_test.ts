...
Deno.test("extractRequestInfo - IP addresses should return Path style", () => {
  ...
  for (const testCase of testCases) {
    ...
    assertEquals(result.urlFormat, urlFormatStyle.def.entries.Path, 
...
Deno.test("extractRequestInfo - virtual hosted style domains", () => {
  const testCases = [
    { url: "http://mybucket.s3.amazonaws.com/object", host: "mybucket.s3.amazonaws.com" },
    { url: "http://test-bucket.s3.amazonaws.com/path/to/object", host: "test-bucket.s3.amazonaws.com" },
    { url: "http://bucket123.s3.us-east-1.amazonaws.com/object", host: "bucket123.s3.us-east-1.amazonaws.com" },
  ];

  for (const testCase of testCases) {
    const request = createMockRequest(testCase.url, testCase.host);
    const result = extractRequestInfo(request);
    assertEquals(result.urlFormat, urlFormatStyle.def.entries.VirtualHosted, 
...
Deno.test("extractRequestInfo - path style domains", () => {
  const testCases = [
    ...
    { url: "http://herald.example.com/bucket/object", host: "herald.example.com" },
    { url: "http://storage.example.com/bucket/object", host: "storage.example.com" },
...
Deno.test("extractRequestInfo - subdomain with forcePathStyle should return Path style", () => {
  ...
  const testCases = [
    ...
    { url: "http://herald.example.com/bucket/object", host: "herald.example.com" },
    { url: "http://minio.example.com/bucket/object", host: "minio.example.com" },
  ];
...
Deno.test("extractRequestInfo - missing host header should throw error", () => {
  ...
  assertThrows(() => extractRequestInfo(request), HeraldError, 
...
Deno.test("Subdomain hosting with forcePathStyle - Issue reproduction", () => {
  ...
  for (const testCase of subdomainTestCases) {
    ...
    assertEquals(requestInfo.urlFormat, testCase.expectedFormat, 
      `${testCase.description}: URL format should be Path`);
    assertEquals(requestInfo.bucket, testCase.expectedBucket,
      `${testCase.description}: Bucket name should be extracted correctly`);
    assertEquals(requestInfo.objectKey, testCase.expectedObjectKey,
...
Deno.test("extractMethod - Valid HTTP methods", () => {
  ...
  for (const method of validMethods) {
    ...
    const requestInfo = extractRequestInfo(request);
    assertEquals(requestInfo.method, method, 
...
Deno.test("extractMethod - Case insensitive method handling", () => {
  ...
  for (const testCase of testCases) {
    ...
    const requestInfo = extractRequestInfo(request);
    assertEquals(requestInfo.method, testCase.expected, 
...
Deno.test("extractRequestInfo - Query parameter extraction", () => {
  ...
  for (const testCase of testCases) {
    const request = createMockRequest(testCase.url, testCase.host);
    const requestInfo = extractRequestInfo(request);
    assertEquals(requestInfo.queryParams, testCase.expectedParams, 
...
Deno.test("extractRequestInfo - Complex real-world scenarios", () => {
  const testCases = [
    {
      ...
      url: "http://mybucket.s3.us-west-2.amazonaws.com/path/to/file.txt?versionId=123&response-content-disposition=attachment",
    ...
    },
    {
      ...
      url: "http://minio.example.com:9000/bucket/folder/file.json?X-Amz-Algorithm=AWS4-HMAC-SHA256",
    ...
    },
    {
      ...
      url: "http://herald.example.com/my-bucket/path/to/object?uploadId=abc123&partNumber=1",
      host: "herald.example.com",
      expectedBucket: "my-bucket",
      expectedObjectKey: "path/to/object",
      expectedFormat: urlFormatStyle.def.entries.Path,
  ...
  for (const testCase of testCases) {
    ...
    assertEquals(requestInfo.method, testCase.expectedMethod, 
...
Deno.test("extractRequestInfo - Edge cases and error conditions", () => {
  ...
  assertThrows(() => extractRequestInfo(requestNoHost), HeraldError, 
    "Invalid request: http://example.com/bucket/object");

  // Test with invalid URL - this will fail at Request construction, not in our code
  assertThrows(() => new Request("not-a-valid-url", {
  ...
  assertEquals(requestInfoLongPath.bucket, "a".repeat(1000), 
    "Should handle very long bucket names");
  assertEquals(requestInfoLongPath.objectKey, "b".repeat(1000), 
...
Deno.test("extractRequestInfo - Unicode and international characters", () => {
  const testCases = [
    ...
    {
      url: "http://mybucket.s3.amazonaws.com/folder/%E6%96%87%E4%BB%B6/%D1%84%D0%B0%D0%B9%D0%BB.txt",
      host: "mybucket.s3.amazonaws.com",
      expectedObjectKey: "folder/%E6%96%87%E4%BB%B6/%D1%84%D0%B0%D0%B9%D0%BB.txt"
    }
  ];
