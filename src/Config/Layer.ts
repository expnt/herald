import { Effect, Layer, Context, type Option } from "effect"
import { parse } from "@std/yaml"
import { GlobalConfig, type MaterializedBucket, lookupBucket } from "../Domain/Config.ts"
...
export const AppConfigLive = Layer.effect(
  AppConfig,
  Effect.gen(function* () {
    const configPath = yield* Effect.succeed(Deno.env.get("CONFIG_PATH") ?? "herald.yaml")

    const content = yield* Effect.tryPromise({
      try: () => Deno.readTextFile(configPath),
      catch: (e) => new Error(`Failed to read config file at ${configPath}: ${e}`)
    })
